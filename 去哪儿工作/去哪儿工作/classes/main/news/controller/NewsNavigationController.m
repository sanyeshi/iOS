...
@interface NewsNavigationController ()<UINavigationControllerDelegate>
{
    CGRect _tabBarFrame;
}
@end
...
{
    [super viewDidLoad];
    self.rootViewController=[[NewsRootViewController alloc] initWithStyle:UITableViewStyleGrouped];
    self.delegate=self;
}
...
{
    //设置tabbarItem
    UIImage * image=[UIImage imageNamed:@"tabbar_info.png"];
    //UIImage * selectedImage=[UIImage imageNamed:@"tabbar_music_selected.png"];
    //[image imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
    //[selectedImage imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
    
    self.tabBarItem=[[UITabBarItem alloc] initWithTitle:@"资讯" image:image selectedImage:image];
}
...
- (void)navigationController:(UINavigationController *)navigationController willShowViewController:(UIViewController *)viewController animated:(BOOL)animated
{
    ...
    if (count>1)
    {
        UITableViewController * rootViewController=(UITableViewController *)self.rootViewController;
        ...
        if (viewController==secondViewController)
        {
            if (self.tabBarController.tabBar.superview!=rootViewController.view)
            {
                ...
                newTabBarFrame.origin.y+=rootViewController.tableView.contentOffset.y;
