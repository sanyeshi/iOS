...
#define kSSLViwepagerImageHeight 160
#define kSSLViwepagrTitleHeight 40
...
-(void) setImages:(NSArray *)models
{
    ...
    if (count>0)
    {
        ...
        SSLViwepagerModel * model=models[count-1];
...
{
    //滚动视图
    _scrollImageView.frame=CGRectMake(0, 0, self.frame.size.width, kSSLViwepagerImageHeight);
    ...
    CGFloat imageViewHeight=kSSLViwepagerImageHeight;
    ...
    //标题栏
    _titleView.frame=CGRectMake(0, CGRectGetMaxY(_scrollImageView.frame), self.frame.size.width, kSSLViwepagrTitleHeight);
    _titleLabel.frame=CGRectMake(kMargin, 0, self.frame.size.width-kPageControllerWidth-kMargin, kSSLViwepagrTitleHeight);
...
-(void) scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    SSLViwepagerModel * model=_models[index];
    _titleLabel.text=model.title;
}
...
{
    if (_delegate&&[_delegate conformsToProtocol:@protocol(SSLViwepagerDelegate)])
    {
        [_delegate sslViwepager:self didSelectAtIndex:_pageControll.currentPage];
    }
}
